...
```yaml
defaults:
  window_hours: 24     # how far back to keep
  max_items: 10        # item cap per source
  order: feed          # feed = keep the feed's own order; recent = newest first

sources:
  - source: BBC News
    category: News
    url: https://feeds.bbci.co.uk/news/rss.xml
    # e.g., use max_items: 20 to override the default limit
```

## Data

RSS data is fetched periodically based on the UTC `cron` schedule in
`.github/workflows/fetch.yaml` and filtered based on the settings in `config.yaml`. The intention is to provide a limited "daily headline briefing" rather than live updating, infinite scroll news site.
